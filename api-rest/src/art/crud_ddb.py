...
async def create_layer(schema: schemas.LayerCreate) -> models.LayerTable:
...
# 1 Query
async def get_artwork_from_layer(layer: models.LayerTable) -> schemas.Artwork:
    try:
    ...
    for sorted_layer in sorted(layer_list, key=lambda x: x.created_at, reverse=True):
        sorted_layer_list.append(schemas.Layer(**sorted_layer.attribute_values))

    return schemas.Artwork(layers=sorted_layer_list, **layer.attribute_values)


# 2 Queries
async def get_artwork_from_id(layer_id: str) -> schemas.Artwork:
    try:
